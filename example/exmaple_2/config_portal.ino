...
void startConfigPortal() {
  configStartMillis = millis(); // start counter开始计时

  WiFi.disconnect();//断开wifi
  WiFi.mode(WIFI_AP);//配置为AP模式
  //WiFi.mode(WIFI_AP_STA);
  IPAddress ap_ip(10, 10, 10, 1);//IP地址
  WiFi.softAPConfig(ap_ip, ap_ip, IPAddress(255, 255, 255, 0));
  String ap_name = AP_NAME + macLastThreeSegments(mac);//wifi名为AP_NAME的内容+mac后三段
  WiFi.softAP(ap_name.c_str());//开启无线网络(c_str()取字符串首地址)
  IPAddress ip = WiFi.softAPIP();//获取模块IP
  Serial.print("IP address: ");
  Serial.println(ip);
  httpUpdateServer.setup(&server, update_path, update_username, update_password);
  server.on("/", handleRoot);//设置HTTP请求回调函数,当访问根路径(“/”)时，会调用handleRoot函数
  server.begin();//启动服务器进行请求监听
//WiFi.beginSmartConfig();
  // unsigned int lastTest = 0;
  // while (deviceMode == CONFIG_MODE) { // BLOCKING INFINITE LOOP直到配置超时
  ...
  //   }
  //   clear_Display();
  //      char2Arr('S', 29, 0);//显示smart config进入配网模式
  //      char2Arr('-', 23, -1);
  //      char2Arr('c', 17, 0);
  //      char2Arr('o', 12, 0);
  //      char2Arr('n', 6, 0);
  //      refresh_display();
  //   if (millis() > lastTest + 50) {//显示剩余配置时间
  //     int splitTime[4] = {
...
void handleRoot() {
  if (server.args()) { //获取参数数量

    // Turn off the nixies
//    ticker.detach();//停止定时调用函数
//    digitalWrite(5, 0);
//    digitalWrite(4, 0);

    if (server.hasArg("ssid")) { //检查参数是否存在
      json["ssid"] = server.arg("ssid");
    }
    if (server.hasArg("pass") && server.arg("pass") == "placeholder") {//占位符
  ...
  html += "<h2>Network settings</h2><p></p> <div class=\"row\"> <label for=\"ssid\">WiFi SSID</label> <input type=\"text\" id=\"ssid\" name=\"ssid\" value=\"";
  ...
  */
  ...
  html += "<h2>Clock settings</h2><p></p>";//<!--<p>Some visual settings and tweaks.</p>-->
  ...
  // html += ">High</option>";
  // html += "</select></div>";
  html += "<div class=\"row\"><label for=\"bri\">Brightness:0-10</label>";
  html += "<input type=\"number\" id=\"bri\" name=\"bri\" min=\"0\" max=\"10\" value=\"";
  html += json["bri"].as<const char*>();
  html += "\"></div>";

  html += "<div class=\"row\"><label for=\"zero\">Scrolling screen</label>";
  html += "<select id=\"zero\" name=\"zero\">";
  unsigned int zero = json["zero"].as<unsigned int>();
  html += "<option value=\"0\"";
  if (zero == 0) html += " selected";
  ...
  html += "<input type=\"number\" id=\"std_offset\" name=\"std_offset\" min=\"-720\" max=\"840\" value=\"";
  ...
  html += "<input type=\"number\" id=\"dst_offset\" name=\"dst_offset\" min=\"-720\" max=\"840\" value=\"";
